  ...
  return new Promise((resolve, reject) => {
    ...
    if(path){
      if(typeof path === 'string'){
        if(path === '*' || path === '/') scanFiles('./').then(files=>resolve(files)).catch(err=>reject(err))
        else {
    ...
    else scanFiles('./').then((files)=>resolve(files))
    .catch(err=>reject(err))
  })
}
...
function uploadUpdatedFiles(domain, files, resetCache, updates, dir, cli){
  return new Promise((resolve, reject) => {
    let customBar = cli? cli.progress({
      barCompleteChar: '\u2588',
      barIncompleteChar: '\u2591',
      format: 'Uploading | '+'{bar}'+' | Files: {value}/{total} | ETA: {eta}s'
    }) : null
    let config_file = {}
    let size = 0
    let cacheArr = []
    for(let path of files) config_file[path] = {updated:fs.statSync(path).mtimeMs, uploaded:null}
    if(cli) customBar.start(files.length, 0, {speed: "N/A"});
    else console.log('Uploading files . . .')
    ...
    createFile('./torus/uploads.json', JSON.stringify(config_file)).then(file => {
      ...
      for(let c in config_file) {
        ...
        if(!updates || config_file[c].updated > config[c].uploaded) {
          upload(domain, c, dir).then((filepath)=>{
            size += 1
            if(config[c].uploaded) cacheArr.push(c)
            cli? customBar.update(size): console.log('uploaded '+ filepath)
            config[c].uploaded = new Date().getTime()
            if(size > Object.keys(config_file).length -1) {
              customBar.stop()
              ...
              if(cacheArr.length>0){
                if(resetCache){
                  ...
                  invalidateCache.aws(domain, cacheArr).then(()=>{
                    cli.action.stop()
                    resolve('All Done!')
                  }).catch(err=>reject(err))
                }
              ...
              else resolve('No updates detected')
            }
          }).catch(err=>reject(err))
        }
        else size +=1
