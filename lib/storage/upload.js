...
function uploadFiles(domain, files, dir, cli){
  return new Promise((resolve, reject) => {
    if(cli){
...
function uploadUpdatedFiles(domain, cli){
  return new Promise((resolve, reject) => {
    getFiles('/').then(files => {
      let config_file = {}
      for(let file of files){
        let file_stat = fs.statSync(file)
        config_file[file] = {updated:file_stat.mtimeMs, uploaded:null}
      }
      createFile('./torus_config/uploads.json').then(file => {
        let config = JSON.parse(file)
        let size = 0
        for(let c in config_file) {
          size += 1
          if(!config[c]) config[c] = config_file[c]
          if(config[c].updated > config[c].uploaded) {
            upload(domain, c, '/').then(()=>{
              if(config[c].uploaded) {
                invalidateCache.aws(c).then(()=> {
                  config[c].uploaded = new Date().getTime()
                  if(size > config.keys().length -1) resolve('All Done!')
                })
              }
              else {
                config[c].uploaded = new Date().getTime()
